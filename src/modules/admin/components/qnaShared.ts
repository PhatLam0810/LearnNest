...
// bảng màu spec: Chờ trả lời (#b45309/#fef3e2), Quá hạn (#c81e1e/#fdecec),
// Đã trả lời (#15803d/#e9f9ef), Đã bỏ qua (#6b7280/#f1f3f7).
export const questionState = (
  item: Pick<QuestionItem, 'isAnswered' | 'dismissedAt' | 'createdAt'>,
) => {
  if (item.isAnswered) {
    return { label: 'Đã trả lời', color: '#15803d', bg: '#e9f9ef' };
  }
  if (item.dismissedAt) {
  ...
  return overdue
    ? { label: 'Quá hạn', color: '#c81e1e', bg: '#fdecec' }
